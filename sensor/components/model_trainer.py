    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        try:
            ...
            if classification_train_metrics.f1_score <= self.model_trainer_config.expected_accuracy:
                raise Exception('Trained model is not good enough to provide epected accuracy')
            
            y_test_pred = model.predict(x_test)
            classification_test_metrics = get_classification_score(y_test, y_test_pred)

            # Check overfitting and Underfitting
            diff = abs(classification_train_metrics.f1_score - classification_test_metrics.f1_score)
